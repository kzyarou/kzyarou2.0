  ...
  return (
    <div className="min-h-screen bg-abyss text-cream">
      <header className="sticky top-0 z-40 bg-abyss/90 backdrop-blur-md border-b border-forest">
        <div className="max-w-[1600px] mx-auto px-4 md:px-12 py-4 md:py-5 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3 md:gap-4 flex-wrap">
            ...
            <span className="hidden sm:inline w-px h-4 bg-forest" />
          ...
          <div className="flex items-center gap-3 md:gap-4">
            <span className="hidden sm:inline font-mono text-[10px] text-cream-dim tracking-wider truncate max-w-[200px]">
              {user.email}
            </span>
            <button
...
function VisitsTab({
  ...
  return (
    <div>
      ...
      <ul className="divide-y divide-forest border-y border-forest">
        <li className="py-3 flex items-center gap-3 bg-deep/40">
          <Checkbox
            checked={allVisibleSelected}
            onChange={toggleAllVisible}
            ariaLabel="Select all visible" />
          
          <span className="font-mono text-[9px] tracking-widest uppercase text-cream-dim/60">
        ...
        <li
          key={v.id}
          className={`py-4 transition-colors ${selected.has(v.id) ? 'bg-amber/5' : ''}`}>
            ...
            <div className="md:hidden flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 mb-1">
                  <span className="font-mono text-sm text-cream truncate">
                    {v.path}
                  </span>
                  <span className="font-mono text-[10px] text-cream-dim shrink-0">
                    {formatTime(v.createdAt)}
                ...
                <div className="font-mono text-[10px] text-cream-dim truncate">
                  {v.referrer || '— direct —'}
                </div>
                <div className="font-mono text-[10px] text-cream-dim/60 truncate mt-0.5">
            ...
            <div className="hidden md:grid grid-cols-12 gap-3 items-center">
              ...
              <span className="col-span-3 font-mono text-sm text-cream truncate">
                {v.path}
              </span>
              <span className="col-span-4 font-mono text-[10px] text-cream-dim truncate">
                {v.referrer || '— direct —'}
              </span>
              <span className="col-span-3 font-mono text-[10px] text-cream-dim/60 truncate">
                {shortenUA(v.userAgent)}
              </span>
              <span className="col-span-1 font-mono text-[10px] text-cream-dim text-right">
                {formatTime(v.createdAt)}
              </span>
            </div>
          </li>
        ...
        {items.map((c) => {
          ...
          return (
            <motion.li
              ...
              <div
                className={`mb-4 flex flex-col md:flex-row md:items-start md:justify-between gap-3 md:gap-4 ${selection ? 'pl-8' : ''} ${!c.read ? 'pr-16 md:pr-0' : ''}`}>
                ...
                <div className="min-w-0">
                  ...
                  <div
                    className={`font-serif font-light text-cream leading-tight break-words ${compact ? 'text-xl' : 'text-2xl md:text-3xl'}`}>
                ...
                <div className="md:text-right shrink-0 flex md:flex-col gap-3 md:gap-0 flex-wrap items-center md:items-end font-mono text-[10px] text-cream-dim/80">
                  {c.company &&
                  <div className="tracking-widest uppercase text-cream-dim">
                      {c.company}
                    </div>
                  }
                  {c.budget &&
                  <div className="text-xs text-amber-bright md:mt-1">
                      {c.budget}
                    </div>
                  }
                  <div className="text-cream-dim/60 md:mt-1">
